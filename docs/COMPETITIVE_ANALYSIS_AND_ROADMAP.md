...
1. T21：为默认仓库增加 selection/status MCP resources，保留 tool 等价路径；
2. T22：增加 refs/author/message 过滤和可分页提交搜索；
...
1. T25：统一 action plan receipt，将 repo/head/index/status 指纹绑定到动作计划；
