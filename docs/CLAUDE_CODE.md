...
- `git_revalidate_plan`: verify a reset/action receipt against current Git state.
...
Legacy tool results contain `schemaVersion: 1`; context, search, diff, history,
and plan validation results use schema version 2 where applicable. Expected
failures use `isError: true` with a stable error code.
...
Windows and lists all twelve tools plus both resources.
