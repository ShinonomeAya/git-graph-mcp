...
- eleven MCP tool definitions, two read-only resources, and two safe-action tools;
...
- the official SDK client completes initialize, lists all eleven tools and both resources, and closes cleanly on Windows;
...
- Official SDK stdio server with eleven tools and two read-only resources.
