# Capability Map: git-graph-mcp v0.2
...
| `mcp-server` | Expose stable MCP tools through the official local stdio transport | `git-domain`, `selection-state` | [SPEC-mcp-server.md](SPEC-mcp-server.md) |
...
| `release-engineering` | Provide tests, diagnostics, portable configuration, CI, npm packaging, and release checks | all modules | [SPEC-release-engineering.md](SPEC-release-engineering.md) |
...
- The terminal remains the primary UI; no Web UI is in v0.2.
...
- Soft, mixed, and hard reset are preview-only. The project must not execute them in v0.2.
...
The v0.2 initiative is complete only when every module success criterion passes, the checkpoints in `tasks/todo.md` are checked, Windows and Linux CI are green, and the human has approved the release candidate. npm publication is a separate, explicit action.
